...
export abstract class BaseEndpointHandler {
  ...
  /**
   * Cache endpoint result with explicit cache type
   */
  protected async cacheResult(cacheKey: string, result: any, context: { domain: string; endpoint: string }, cacheType: CacheType): Promise<void> {
    await this.cache.set(cacheKey, result, cacheType);
